...
export default function SummaryCards({ data }: { data: Transaction[] }) {
  ...
  const showCards =
    params.get("showCards") === "true" || !params.has("showCards");
              ...
              <p className="text-sm text-muted-foreground">
                {results.yesterday.total / (results.today.total || 1) > 1 ? (
                  <span className="text-red-500 text-sm flex">
                    {(
                      results.yesterday.total / (results.today.total || 1)
                    ).toFixed(2)}
                    % <ArrowDownIcon /> ontem
                  </span>
                ) : (
                  <span className="text-green-500 text-sm flex items-center">
                    {(
                      results.today.total / (results.yesterday.total || 1)
                    ).toFixed(2)}
                    % <ArrowUpIcon />
                    ontem
                  </span>
            ...
            <CardFooter>
              <p className="text-sm text-muted-foreground">
                {results.lastWeek.total / (results.week.total || 1) > 1 ? (
                  <span className="text-red-500 text-sm flex">
                    {(
                      results.lastWeek.total / (results.week.total || 1)
                    ).toFixed(2)}
                  ...
                  <span className="text-green-500 text-sm flex items-center">
                    {(
                      results.week.total / (results.lastWeek.total || 1)
                    ).toFixed(2)}
                    % <ArrowUpIcon />
                    semana passada
                  </span>
            ...
            <CardFooter>
              <p className="text-sm text-muted-foreground">
                {results.lastMonth.total / (results.month.total || 1) > 1 ? (
                  <span className="text-red-500 text-sm flex">
                    {(
                      results.lastMonth.total / (results.month.total || 1)
                    ).toFixed(2)}
                  ...
                  <span className="text-green-500 text-sm flex items-center">
                    {(
                      results.month.total / (results.lastMonth.total || 1)
                    ).toFixed(2)}
